...
# ----------------------- alignment_broken
# Given an alignment that has been read in by the seqinr function,
# check if the sequences are obviously broken.
alignment_broken  <- function(a) {
    len1 <- length(a[[1]])
    s <- "First seq length %d, but seq \"%s\" length %d"
    for ( aa in a) {
        if (length(aa) != len1) {
            message (sprintf (s, len1, attr(aa, "Annot"), length(aa)))
            return(T)
        }
    }
    return (F)
...
presence_names <- function(fstafname) {
    a <- seqinr::read.fasta(fstafname)
    if (alignment_broken(a))
        stop(paste("alignment from ", fstafname), "broken")
...
mymain <- function() {
    ...
    fstafname <- dlg_open(title = "Select a Fasta file.")$res
    if (nchar(fstafname) < 1) {
        stop("Did not get a valid path for sequence file")
    }
    t <- presence_names (fstafname)
    nameset <-  t$nameset
    presence <-  t$presence
...
#   Ask which taxonomic rank he would like to select (and checked for typos)
    ...
    while (taxrank != "kingdom" && taxrank != "phylum" &&
           taxrank != "class" && taxrank != "order" && taxrank != "family" && taxrank != "genus") {
...
#   sort the groups from most seen group to less seen group
    ...
    nametaxacopy <- transform(nametaxacopy, freq = ave(seq(nrow(nametaxacopy)),
                                                       taxlevel, FUN=length))
    nametaxacopy <- nametaxacopy[order(nametaxacopy$freq, decreasing = T), ]
